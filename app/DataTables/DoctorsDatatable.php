    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('status', function ($query) {
                if ($query->status == 1) {
                    return '<a class="bg-blue text-white px-2" href="' . route("doctor.status", $query->id) . '">' . __('Active') . '</a>';
                } elseif ($query->status == 0) {
                    return '<a class="bg-yellow text-white px-2" href="' . route("doctor.status", $query->id) . '">' . __('Inactive') . '</a>';
                }
            })
            ->addColumn('approve', function ($query) {
                if ($query->approve == 1) {
                    return '<a class="bg-green text-white px-2" href="' . route("doctor.approve", $query->id) . '">' . __('Approved') . '</a>';
                } else {
                    return '<a class="bg-red text-white px-2" href="' . route("doctor.approve", $query->id) . '">' . __('Not Approve') . '</a>';
                }
            ...
            ->rawColumns(['image', 'action', 'status', 'approve']);
    }
